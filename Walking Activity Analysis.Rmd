...
As illustrated below, the histogram is broken out into 5 bins with the median and mean overlaping. However, there is a 1 unit difference where the median is closer to 0 demonstrating a slight skewness of the data. Due to the presence of NA values in the dataset, I ignore them to evaluate the current population of true values. The total.steps.day is a summation of the steps for each day in the dataset.
```{r}
total.steps.day <- tapply(raw.data$steps, raw.data$date, sum)
hist(total.steps.day, main = "Histogram of Total Number of Steps Taken Each Day", xlab = "Total Steps", ylim = c(0,30))
...
hist(total_steps_day, main = "Histogram of Total Number of Steps Taken Each Day", xlab = "Total Steps")
